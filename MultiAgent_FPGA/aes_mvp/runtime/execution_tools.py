"""Custom SDK tools that execute AES MVP framework actions structurally."""
...
from MultiAgent_FPGA.aes_mvp.delegation import DelegateBatchTask, SubagentWorkMode
...
from MultiAgent_FPGA.aes_mvp.runtime.sdk_shim import SDKUnavailableError
...
def _dispatch_executor(
    ...
    from MultiAgent_FPGA.aes_mvp.__main__ import (
        _generate_node,
        _record_repair_edit,
        _run_integration,
        _run_node,
...
class RunExecutorTool(ToolDefinition[RunExecutorAction, RunExecutorObservation]):
    ...
    def create(cls, conv_state=None, **params) -> Sequence['RunExecutorTool']:  # noqa: ARG003
        return [
            cls(
                description=(
                    'Run the framework-owned AES MVP executor for one task contract. '
