...
from MultiAgent_FPGA.aes_mvp.runtime.execution_tools import register_execution_tools
from MultiAgent_FPGA.aes_mvp.synthesis import FragilityMemory, L2AdaptivePlanner
...
def _l2_enabled() -> bool:
    ...
    return os.environ.get('AES_MVP_ENABLE_L2', '').lower() in ('1', 'true', 'yes')
...
class SdkAgentFactory:
    ...
    def create_execution_orchestrator(self, *, enable_l2: bool | None = None) -> Any:
        ...
        llm = self._build_llm(
            spec.llm_profile, usage_id='aes_mvp.orchestrator.execution'
        )
        condenser: Any | None = None
        disable = os.environ.get('AES_MVP_DISABLE_EXECUTION_CONDENSER', '').lower()
        if disable not in ('1', 'true', 'yes'):
            from openhands.sdk.context.condenser import LLMSummarizingCondenser

            max_size = int(os.environ.get('AES_MVP_EXECUTION_CONDENSER_MAX_SIZE', '12'))
            keep_first = int(
                os.environ.get('AES_MVP_EXECUTION_CONDENSER_KEEP_FIRST', '2')
            )
            condenser = LLMSummarizingCondenser(
                llm=llm.model_copy(
                    update={'usage_id': 'aes_mvp.orchestrator.execution.condenser'}
                ),
                max_size=max_size,
        ...
        return self._create_agent_from_spec(
            spec,
            usage_id='aes_mvp.orchestrator.execution',
    ...
    def create_finalizer_orchestrator(self) -> Any:
        ...
        return self._create_agent_from_spec(
            spec,
            usage_id='aes_mvp.orchestrator.finalizer',
    ...
    def _build_worker_condenser(
        ...
        disable = os.environ.get('AES_MVP_DISABLE_WORKER_CONDENSER', '').lower()
    ...
    def create_module_repair_worker(self, node: PlanDAGNode) -> Any:
        spec = build_repair_worker_spec(module_id=node.module_id)
        usage_id = (
            f'aes_mvp.module_repair_worker.{_sanitize_registry_name(node.module_id)}'
        )
        llm = self._build_llm(spec.llm_profile, usage_id=usage_id)
        return self._create_agent_from_spec(
            spec,
