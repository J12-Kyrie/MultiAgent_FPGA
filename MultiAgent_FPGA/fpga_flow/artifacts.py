...
class SpecIR(BaseModel):
    ...
    def validate_design_scope(self) -> 'SpecIR':
        if not self.algorithm:
            raise ValueError('SpecIR.algorithm must be a non-empty string')
        if not self.variant:
            raise ValueError('SpecIR.variant must be a non-empty string')
...
class PlanDAG(BaseModel):
    ...
    @model_validator(mode='after')
    def validate_frozen_dag(self) -> 'PlanDAG':
