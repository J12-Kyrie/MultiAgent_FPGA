...
from MultiAgent_FPGA.fpga_flow.paths import DESIGNS_DIR, PACKAGE_ROOT
...
from MultiAgent_FPGA.fpga_flow.skill_refs import select_skill_refs
...
def resolve_vector_path(vector_set: str) -> Path | None:
    """Return the first existing vector file for common layouts."""
    vs = str(vector_set).strip()
    if not vs:
        return None
    rel = Path(vs)
    if rel.is_absolute():
        return rel.resolve() if rel.is_file() else None
...
def _vector_implementation_hints(
    module_id: str,
    vector_set: str,
    *,
    ...
    if not examples:
        return []
    # Check for a vector_hint_style override in design_context (blueprint-driven).
    hint_style = ''
    if design_context:
        hint_style = str(design_context.get('vector_hint_style', ''))
    if hint_style == 'hex_pair_lookup':
        formatted_pairs = []
    ...
    return [
        {
            ...
            'vector_set': 'vectors/aes_sbox_kat.txt',
            ...
            'design_context': {
                ...
                'vector_hint_style': 'hex_pair_lookup',
                'l2_profiles': [],
                'l2_defaults': {},
                'sink_role': False,
            },
        },
        {
            ...
            'criticality': 'high',
            'integration_role': 'leaf',
            'vector_set': 'vectors/aes_key_schedule_128_kat.txt',
            ...
            'design_context': {
                ...
                'l2_defaults': {
                    'rand_small': {
                        'vecfile': 'vectors/aes_key_schedule_128_kat.txt',
                        'cases': 8,
                        'seed': 7,
                    },
                    'rand_medium': {
        ...
        },
        {
            ...
            'criticality': 'high',
            'integration_role': 'leaf',
            'vector_set': 'vectors/aes_round_transform_kat.txt',
            ...
            'design_context': {
                ...
                'worker_prompt_hint': 'MixColumns byte-packing uses a deliberate double-reversal. Do not simplify the concatenation order.',
                'ports': [
                    _port(
                        'state_in',
                        'input',
                ...
                'l2_defaults': {
                    'rand_small': {
                        'vecfile': 'vectors/aes_round_transform_kat.txt',
                        'cases': 8,
                        'seed': 7,
                    },
                    'rand_medium': {
        ...
        },
        {
            ...
            'vector_set': 'vectors/aes128_encrypt_core_kat.txt',
            ...
            'design_context': {
                ...
                'l2_defaults': {
                    ...
                    'rand_medium': {
                        'vecfile': 'vectors/aes128_encrypt_core_l2_rand_medium.txt',
                    },
                    'back_to_back': {
                        'vecfile': 'vectors/aes128_encrypt_core_l2_back_to_back.txt',
                    },
                    'mid_reset': {
                        'vecfile': 'vectors/aes128_encrypt_core_l2_mid_reset.txt',
                    },
                },
                'repair_hints': {
                    'must_add_tokens': [
                    ...
                    'first_edit_steps': [
                        ...
                        'Keep the datapath hook local to this module for the MVP; you may use a local encrypt-block helper function or equivalent behavioral round helper.',
                    ],
                    'extra_vector_files': [
                ...
                'integration_vector_set': 'vectors/aes128_encrypt_core_regress.txt',
...
def synthesize_spec_ir(
    *,
    ...
    if blueprint_path is not None:
        from MultiAgent_FPGA.fpga_flow.blueprint_loader import BlueprintLoader

        blueprint = BlueprintLoader.load(Path(blueprint_path))
        return BlueprintLoader.to_spec_ir(blueprint, system_goal=system_goal)
...
def synthesize_plan_dag(
    ...
    *,
    ...
    if blueprint_path is not None:
        ...
        blueprint = BlueprintLoader.load(bp)
        return BlueprintLoader.to_plan_dag(
            blueprint, spec_ir, design_root=design_root or bp.parent
        )

    resolved_design_root = design_root or (DESIGNS_DIR / 'aes128')
    nodes: list[PlanDAGNode] = []
    for entry in _aes_blueprints():
        rtl_files = [
            str((resolved_design_root / f).resolve()) for f in entry['rtl_files']
        ]
        tb_file = str((resolved_design_root / entry['tb_file']).resolve())
        vector_set = (
            str((resolved_design_root / entry['vector_set']).resolve())
            if entry['vector_set']
            else ''
        )
        nodes.append(
            PlanDAGNode(
                ...
                design_context={
                    **entry['design_context'],
                    'scope': 'AES-128 encrypt-only block-handshake iterative-10-round MVP',
                    'design_root': str(resolved_design_root),
...
def synthesize_integration_manifest(
    ...
    *,
    ...
    if blueprint_path is not None:
        ...
        blueprint = BlueprintLoader.load(Path(blueprint_path))
        result = BlueprintLoader.to_integration_manifest(blueprint, spec_ir, plan_dag)
        if result is None:
            raise ValueError('Blueprint has no integration-ready top module')
        return result
...
class ContractCompiler:
    ...
    def compile_design_brief(
        ...
        *,
        ...
        skill_paths = [str(ref.path) for ref in select_skill_refs(*resolved_keys)]
        ...
        # Read top-module implementation hints from design_context if available.
        top_hints = list(context.get('top_implementation_hints', []))
        if node.integration_role == 'top' and top_hints:
            implementation_hints.extend(top_hints)
        elif node.integration_role == 'top':
            # Fallback: generic top-module guidance
            implementation_hints.append(
                'Ensure the top-level handshake, latency contract, and '
                'checkpoint set are correct before optimizing submodule logic.'
            )
        ...
        # Read top-module repair hints from design_context if available.
        top_repair_hints = list(context.get('top_repair_hints', []))
        if node.integration_role == 'top' and top_repair_hints:
            repair_hints.extend(top_repair_hints)
        elif node.integration_role == 'top':
            repair_hints.append(
                'Prioritize the top-level handshake and latency contract '
                'before micro-optimizing submodule logic.'
            )
        repair_hints.extend(
            _vector_implementation_hints(
                node.module_id,
