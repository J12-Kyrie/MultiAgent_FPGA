"""CLI entrypoint for the FPGA flow framework package."""
...
import MultiAgent_FPGA.fpga_flow._config as _config
...
from MultiAgent_FPGA.fpga_flow.llm_profiles import run_deepseek_preflight_suite
...
def _validate_summary(
    *,
    ...
    bootstrap = RuntimeBootstrap.build(
        allow_placeholder_api_key=True,
...
def _run_node(
    ...
    bootstrap = RuntimeBootstrap.build(
        allow_placeholder_api_key=True,
        blueprint_path=_config.ACTIVE_BLUEPRINT_PATH,
    )
...
def _run_fpga_flow(message: str, dry_run: bool) -> dict[str, object]:
    bootstrap = RuntimeBootstrap.build(
...
def _run_design(
    ...
    _config.ACTIVE_BLUEPRINT_PATH = blueprint_path
    bootstrap = RuntimeBootstrap.build(
        allow_placeholder_api_key=dry_run,
        system_goal=message,
        blueprint_path=blueprint_path,
    ...
    if dry_run:
        summary = bootstrap.to_summary()
        summary['dry_run'] = True
        summary['execution_mode'] = 'blueprint-driven'
        summary['blueprint'] = str(Path(blueprint_path).resolve())
        return summary

    session = ExecutionSession.create(bootstrap)
    return asdict(session.run(message or bootstrap.system_goal))


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog='python -m MultiAgent_FPGA.fpga_flow')
    subparsers = parser.add_subparsers(dest='command', required=False)

    validate_cmd = subparsers.add_parser('validate')
    validate_cmd.add_argument('--blueprint', default=None)
    ...
    run_design = subparsers.add_parser('run-design')
    run_design.add_argument(
        '--blueprint',
        required=True,
        help='Path to a design blueprint YAML file.',
    )
    run_design.add_argument('--message', default=None)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if command == 'validate':
        payload = _validate_summary(blueprint_path=getattr(args, 'blueprint', None))
        ...
        if args.enable_l2:
            os.environ['FPGA_FLOW_ENABLE_L2'] = '1'
        payload = _run_fpga_flow(args.message, args.dry_run)
    elif command == 'run-design':
        if args.enable_l2:
            os.environ['FPGA_FLOW_ENABLE_L2'] = '1'
        payload = _run_design(args.blueprint, args.message, args.dry_run)
    else:
