...
from MultiAgent_FPGA.fpga_flow.delegation import (
    DelegateBatchPlan,
    DelegateBatchTask,
...
from MultiAgent_FPGA.fpga_flow.policy import OrchestratorState
from MultiAgent_FPGA.fpga_flow.runtime.bootstrap import RuntimeBootstrap
from MultiAgent_FPGA.fpga_flow.runtime.context_assembler import (
...
class ExecutionSession:
    ...
    def create(
        ...
        *,
        ...
        enable_l2 = os.environ.get('FPGA_FLOW_ENABLE_L2', '').lower() in (
            '1',
            'true',
            'yes',
        )
    ...
    def _build_autonomous_delegate_plan(self) -> dict[str, Any]:
        ...
        def generate_node_command(module_id: str) -> str:
            ...
            return (
                ...
                f'-m MultiAgent_FPGA.fpga_flow generate-node {module_id} '
        ...
        def node_executor_command(
            ...
            *,
            ...
            command = (
                f'PYTHONPATH={self.bootstrap.repo_root} '
                f'/opt/anaconda3/envs/openhands/bin/python '
                f'-m MultiAgent_FPGA.fpga_flow run-node {module_id}'
            )
            if workspace_root:
        ...
        def build_generate_validate_task(module_id: str) -> DelegateBatchTask:
            ...
            draft_rtl_paths = generation_entry.get('draft_rtl_paths', [])
            memory_populated = any(
                Path(p).is_file() and Path(p).stat().st_size > 0
                for p in draft_rtl_paths
            )
            design_brief_path = generation_entry['contract_paths'].get(
                'design_brief', ''
            )
            draft_rtl_list = generation_entry.get('draft_rtl_paths', [])
            draft_tb = generation_entry.get('draft_tb_path', '')
            if memory_populated:
                draft_guidance = (
                    'The draft files are pre-populated from verified memory. '
                    'Call run_executor to compile and validate.'
                )
            else:
                draft_guidance = (
                    'IMPORTANT: The draft RTL and TB files are EMPTY. You must write complete implementations before calling run_executor.\n'
                    'Follow these steps IN ORDER:\n'
                    f'1. Read the design brief: {design_brief_path}\n'
                    f'2. Write the complete Verilog RTL to: {", ".join(draft_rtl_list)}\n'
                    f'3. Write the complete C++ testbench to: {draft_tb}\n'
                    '4. The testbench must emit CHECKPOINT|<name>|PASS|<detail> lines to stdout for each checkpoint.\n'
                    '5. After writing both files, call run_executor to compile and validate.\n'
                    'Do NOT call run_executor before writing the RTL and TB files — empty files will fail compilation.'
                )
            prompt = '\n'.join(
                [
        ...
        def build_repair_task(module_id: str) -> DelegateBatchTask:
            ...
            manual_receipt_command = (
                f'PYTHONPATH={self.bootstrap.repo_root} '
                f'/opt/anaconda3/envs/openhands/bin/python '
                f'-m MultiAgent_FPGA.fpga_flow record-repair-edit {module_id} '
        ...
        delegate_plan = self._materialize_delegate_plan(
            {
                ...
                'manual_integration_command': (
                    f'PYTHONPATH={self.bootstrap.repo_root} '
                    f'/opt/anaconda3/envs/openhands/bin/python '
                    '-m MultiAgent_FPGA.fpga_flow run-integration '
    ...
    def _run_autonomous_execution_loop(
        ...
        *,
        ...
        has_memory = self._has_memory_populated_drafts()
        _PHASE_MAX_TURNS: dict[str, int] = {
            'generate_validate': 60 if has_memory else 200,
            'repair_edit': 30 if has_memory else 80,
            'revalidate': 25,
            'l2_execute': 30,
            'integration': 40,
        }
        ...
        while True:
            ...
            phase = str(current_batch.get('phase', ''))
            batch_tasks = [
                t for t in current_batch.get('tasks', []) if isinstance(t, dict)
            ]
            batch_modules = [
                str(t.get('request', {}).get('module_id', '')) for t in batch_tasks
            ]
            max_turns = _PHASE_MAX_TURNS.get(phase, 25)

            use_direct_worker = (
                len(batch_tasks) == 1
                and phase in {'generate_validate', 'repair_edit'}
                and not self._has_memory_populated_drafts()
            )
            if use_direct_worker:
                task_prompt = str(batch_tasks[0].get('prompt', ''))
                module_id = batch_modules[0] if batch_modules else ''
                node = next(
                    (
                        n
                        for n in self.bootstrap.plan_dag.nodes
                        if n.module_id == module_id
                    ),
                    None,
                )
                if node is not None and task_prompt:
                    active_mode = (
                        'generate' if phase == 'generate_validate' else 'repair'
                    )
                    batch_conversation = self.runner.create_direct_worker_conversation(
                        node,
                    ...
                    batch_message = task_prompt
                    logger.info(
                        'Using direct worker for %s (phase=%s, module=%s)',
    ...
    def _has_memory_populated_drafts(self) -> bool:
        """Check if any node in the plan has pre-populated memory drafts."""
        from MultiAgent_FPGA.fpga_flow.memory import MemoryStore

        store = MemoryStore(memory_required=False)
        return any(
            store.has_module_memory(node.module_id)
            for node in self.bootstrap.plan_dag.nodes
        )
    ...
    def _autonomous_batch_timeout_s(self, current_batch: dict[str, Any]) -> float:
        ...
        if self.focus_module_id in focus_modules:
            ...
            return 300.0
        if phase in {'generate_validate', 'repair_edit', 'revalidate'}:
            return 900.0 if not has_memory else 420.0
        return 180.0
