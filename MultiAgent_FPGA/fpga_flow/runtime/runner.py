...
_PAUSE_TIMEOUT_S = 10.0


def _safe_pause(conversation, timeout_s: float = _PAUSE_TIMEOUT_S) -> bool:
    """Attempt conversation.pause() with a timeout to avoid deadlock.

    Returns True if pause completed, False if it timed out or failed.
    The SDK's pause() can deadlock when the runner thread holds a lock
    while blocked in a syscall (e.g. waitpid on a dead subprocess).
    """
    ...
    def _do_pause() -> None:
        try:
            ...
            logger.warning('conversation.pause() raised', exc_info=True)
        finally:
            success.set()
    ...
    if not success.is_set():
        logger.error(
            'conversation.pause() did not complete within %.1fs — '
            'possible SDK deadlock, skipping pause',
            timeout_s,
        )
        return False
    return True
...
class ConversationRunner:
    ...
    def create_direct_worker_conversation(
        self,
        node,
        *,
        active_mode: str | None = 'generate',
        conversation_id: UUID | None = None,
        persistence_dir: str | Path | None = None,
        delete_on_close: bool = True,
        max_iteration_per_run: int = 100,
    ):
        """Create a worker conversation directly, bypassing orchestrator delegation.

        For single-module batches this eliminates 3-5 orchestrator LLM calls.
        """
        if active_mode == 'repair':
            worker = self.agent_factory.create_module_repair_worker(node)
        else:
            worker = self.agent_factory.create_module_worker(
    ...
    def run_current_batch_until_gate(
        ...
        *,
        ...
        if runner_thread.is_alive():
            logger.error(
                'Conversation run thread still alive after pause — '
                'returning partial result instead of blocking indefinitely'
            )
        if run_errors:
            raise run_errors[0]
