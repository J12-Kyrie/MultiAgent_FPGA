...
def build_workflow_orchestrator_spec(
    *,
    ...
    return AgentSpawnSpec(
        ...
        system_prompt=build_execution_orchestrator_prompt(design_name=design_name),
        skill_keys=(
            *orchestrator_skill_keys(design_name),
            *SDK_DELEGATION_SKILL_KEYS,
        ),
        allowed_states=(
...
def _design_name_from_node(node: PlanDAGNode) -> str | None:
    name = str(node.design_context.get('algorithm', ''))
...
def build_repair_worker_spec(
    *,
    ...
    return AgentSpawnSpec(
        role=AgentRole.MODULE_WORKER_SUBAGENT,
        llm_profile=LLMProfileName.DEEPSEEK_OFFICIAL_FAST,
        system_prompt=build_repair_worker_prompt(
            module_id=module_id, design_name=design_name
        ),
        skill_keys=select_worker_skill_keys('repair', design_name=design_name),
        allowed_states=(
            OrchestratorState.MODULE_DESIGN,
            OrchestratorState.MODULE_L0,
            OrchestratorState.MODULE_L1,
...
def build_l2_campaign_spec(
    ...
    *,
    ...
    return AgentSpawnSpec(
        ...
        skill_keys=select_worker_skill_keys('l2_execute', design_name=dn),
