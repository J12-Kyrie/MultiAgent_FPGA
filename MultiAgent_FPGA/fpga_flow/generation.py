...
from MultiAgent_FPGA.fpga_flow.executor_contracts import summarize_required_checkpoints
from MultiAgent_FPGA.fpga_flow.paths import PACKAGE_ROOT
from MultiAgent_FPGA.fpga_flow.synthesis import ContractCompiler, resolve_vector_path

DEFAULT_REPAIR_BUDGET = 2
TOP_MODULE_REPAIR_BUDGET = 3
_DEFAULT_FILE_EXCERPT_LINES = 120
_TB_SUPPORT_HEADER_SUFFIXES = ('.hpp', '.h')
...
def _stage_rtl_include_files(
    target_dir: Path, *, design_root: Path | None = None
) -> list[str]:
    """Copy .vh/.svh include files from source RTL into the workspace draft RTL dir."""
    staged: list[str] = []
    target_dir.mkdir(parents=True, exist_ok=True)
    rtl_source = (design_root / 'rtl') if design_root else (PACKAGE_ROOT / 'rtl')
...
def _stage_tb_support_headers(
    target_dir: Path, *, design_root: Path | None = None
) -> list[str]:
    staged: list[str] = []
    target_dir.mkdir(parents=True, exist_ok=True)
    tb_source = (design_root / 'tb') if design_root else (PACKAGE_ROOT / 'tb')
    if not tb_source.is_dir():
        return staged
    for source in tb_source.iterdir():
        if source.suffix.lower() in _TB_SUPPORT_HEADER_SUFFIXES and source.is_file():
            target = (target_dir / source.name).resolve()
            shutil.copyfile(source, target)
            staged.append(str(target))
    return staged
...
def initialize_node_workspace(
    *,
    ...
    # memory_required=False allows non-AES designs to skip memory and generate from scratch.
    from MultiAgent_FPGA.fpga_flow.memory import MemoryStore

    store = MemoryStore(memory_required=False)
    written = store.populate_workspace(node.module_id, workspace_root)

    # Derive design_root from node's design_context for staging source files.
    _dr_str = node.design_context.get('design_root')
    _design_root = Path(_dr_str) if _dr_str else None

    # Fallback: if no memory was available, seed drafts from design reference files.
    if not written and _design_root:
        for rtl_file in node.rtl_files:
            src = _design_root / 'rtl' / Path(rtl_file).name
            if src.is_file():
                dst = workspace_root / 'draft' / 'rtl' / Path(rtl_file).name
                dst.parent.mkdir(parents=True, exist_ok=True)
                if not dst.exists():
                    shutil.copy2(str(src), str(dst))
                    written['rtl'] = dst
        tb_src = _design_root / 'tb' / Path(node.tb_file).name
        if tb_src.is_file():
            tb_dst = workspace_root / 'draft' / 'tb' / Path(node.tb_file).name
            tb_dst.parent.mkdir(parents=True, exist_ok=True)
            if not tb_dst.exists():
                shutil.copy2(str(tb_src), str(tb_dst))
                written['tb'] = tb_dst
    ...
    if draft_rtl_dir:
        _stage_rtl_include_files(Path(draft_rtl_dir).parent, design_root=_design_root)
    ...
    _stage_tb_support_headers(draft_tb_path.parent, design_root=_design_root)
...
def _build_first_edit_steps(
    *,
    ...
    if primary_target_file.endswith('.cpp') and expected_checkpoint:
        ...
        if node.integration_role == 'top':
            # Read must_add_tokens and first_edit_steps from design_context if available.
            context_tokens = list(
                node.design_context.get('repair_hints', {}).get('must_add_tokens', [])
            )
            context_steps = list(
                node.design_context.get('repair_hints', {}).get('first_edit_steps', [])
            )
            if context_tokens:
                must_add_tokens.extend(context_tokens)
            if context_steps:
                first_edit_steps.extend(context_steps)
            else:
                first_edit_steps.extend(
                    [
                        'Install the top-module control skeleton: reset handling, input acceptance, busy/done signaling.',
                        'Ensure the datapath connects submodule outputs to the top-level interface.',
                    ]
                )
...
def _rtl_vector_hint(node: PlanDAGNode) -> str | None:
    ...
    hint_style = str(node.design_context.get('vector_hint_style', ''))
    if hint_style == 'hex_pair_lookup':
        ...
        if mappings:
            return (
                'Implement these frozen KAT mappings in the RTL first: '
                + ', '.join(mappings)
                + '. A case statement covering these entries is acceptable for the current vector set.'
            )
    if node.integration_role == 'top':
        extra_vec_names = list(
            node.design_context.get('repair_hints', {}).get('extra_vector_files', [])
        )
        extra_examples: list[str] = []
        for candidate_name in extra_vec_names:
...
def write_repair_request(
    *,
    ...
    from MultiAgent_FPGA.fpga_flow.artifacts import ValidationFailurePhase
...
def promote_workspace(
    *,
    ...
    if promoted_rtl_dir:
        promoted_rtl_includes = _stage_rtl_include_files(
            promoted_rtl_dir, design_root=_design_root
        )
